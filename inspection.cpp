...
namespace copa
{
    ...
    void Inspection::call_python_class(const std::string &flow_data, const std::string &results)
    {
        ...
        pName = PyUnicode_DecodeFSDefault("consolidate_results"); // Nome do script Python
        ...
        m_consolidation_thread = std::thread([this, consolidation_interval]()
            ...
            while (!m_stop_thread) {
                ...
                if (m_condition.wait_for(lock, std::chrono::seconds(consolidation_interval)) == std::cv_status::timeout) {
                    ...
                    //Test pytboind11 a1ui
                    call_python_class(flow_data.dump(), result.dump());
